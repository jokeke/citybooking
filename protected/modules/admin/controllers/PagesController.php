...
class PagesController extends ControllerModule
{
	/**
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Pages']))
		{
            ...
            $model->news_date = strtotime($model->news_date);
            if($model->save())
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Pages']))
		{
            ...
            if ($file = CUploadedFile::getInstance($model, 'image_upl'))
            {
                //$this->deleteFile($img); // старый файл удалим, потому что загружаем новый
                $filename = 'pages_' . $model->url . '_' . md5(time()) . '.' . $file->extensionName;
                $model->image = '/media/images/' . $filename;
                $file->saveAs(Yii::app()->basePath.'/../media/images/' . $filename);
	...
	public function loadModel($id)
	{
        ...
        $model->news_date = date('d.m.Y', $model->news_date);
		if($model===null)
